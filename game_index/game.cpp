...
float scaleX, scaleY, scale = 1.0f;                // ADDED: globals that hold the current scaling each frame
    ...
    for(size_t y=0;y<lvl1.rows.size();++y)
        for(size_t x=0;x<lvl1.rows[y].size();++x)
            if(lvl1.rows[y][x]=='p'){
                ...
                lvl1.rows[y][x]='.';        // ADDED: erase the 'p' so the grid is floor here
            }
}
// ─────────────────────────────────────────────────────────────────
...
void loadLvl1(){
    ...
    if(!loaded){
        readlvlData();
        cam.offset={GetScreenWidth()/2.0f,GetScreenHeight()/2.0f}; // ADDED: set screen centre
        cam.rotation=0.0f;
        cam.zoom=1.0f;
    ...
    scale  = (scaleX+scaleY)/2.0f;                                  // ADDED: uniform integer-ish scale

    movementEventHandler();

    // animation timer
    bool moving = IsKeyDown(KEY_W)||IsKeyDown(KEY_A)||IsKeyDown(KEY_S)||IsKeyDown(KEY_D); // ADDED: current input state
    if(moving){
    ...
    Rectangle src={currentFrame*16.0f,0,16,16};                     // ADDED: pick frame slice
    Rectangle dst={(float)pPixX,(float)pPixY,(float)pSize,(float)pSize};
    DrawTexturePro(playerTex,src,dst,{0,0},0.0f,WHITE);             // ADDED: blit sprite
    EndMode2D();
}
// ─────────────────────────────────────────────────────────────────
